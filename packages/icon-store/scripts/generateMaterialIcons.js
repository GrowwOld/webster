...
const updateFromGit = () => {
  if (fs.existsSync(gitFolder)) {
    ...
    execSync(
      `git clone https://github.com/google/material-design-icons.git ${gitFolder}`
    );
  }
};
...
function copySvgsFromGit() {
  ...
  for (let category of categoryFolders) {
    const categorySvgFolderPath = path.join(
      materialIconsCategoryPath,
      category
    );
    ...
    for (let svgFolder of svgFolders) {
      ...
      const outputFilePath = path.join(
        "svgs",
        newMaterialSvgPath + normalizedFileName + ".svg"
      );
